...
exports.itemSchema = Joi.object({
    ...
    photoUrls: Joi.array().min(1).max(3).required().items(Joi.string().uri().required()).error(() => new Error('El campo photoUrls debe ser un arreglo de hileras de caracteres, con un mínimo de 1 y un máximo de 3, y el formato de las hileras debe ser una url')),
});